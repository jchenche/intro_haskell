...
p_not_not p =
    case double_negation of
        (Conj p_imp_nnp _) -> p_imp_nnp p

not_not_p :: Not (Not p) -> p
not_not_p nnp =
    case double_negation of
        (Conj _ nnp_imp_p) -> nnp_imp_p nnp

transposition :: (p -> q) <-> (Not q -> Not p)
transposition = Conj dir1 dir2
    where
        dir1 = modus_tollens
        dir2 not_q_imp_not_p p = not_not_p $ modus_tollens not_q_imp_not_p (p_not_not p)
...
de_morgan = Conj dir1 dir2
    where
        dir1 not_p_or_q = Conj not_p not_q
            where
                not_p = \p -> not_p_or_q (Left p)
                not_q = \q -> not_p_or_q (Right q)
        dir2 (Conj not_p not_q) = \p_or_q ->
            case p_or_q of
                (Left p)  -> not_p p
                (Right q) -> not_q q
