...
evalBattle [] bf = bf
evalBattle ((attackerDV, defenderDV):battleDVs) (Battlefield attacker defender) = if attackerDV > defenderDV
                                                                                  then evalBattle battleDVs (Battlefield attacker (defender - 1))
                                                                                  else evalBattle battleDVs (Battlefield (attacker - 1) defender)
...
maxAttackerToDeploy = 3
maxDefenderToDeploy = 2

battle :: Battlefield -> Rand StdGen Battlefield
battle bf@(Battlefield attacker defender) = replicateM (min maxAttackerToDeploy (attacker - 1)) die >>= \attackerDVs ->
                                            replicateM (min maxDefenderToDeploy defender) die >>= \defenderDVs ->
...
successProb bf = replicateM 1000 (invade bf) >>= \battleResults -> return $ (sum $ map interpretWinner battleResults) / 1000

-- test = (evalRandIO $ successProb (Battlefield 11 10)) >>= \x -> print x
